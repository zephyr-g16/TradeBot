...
command_list = ["start", "stop", "status", "list all", "heartbeat", "quit", "shutdown", "stop all"]
...
Hello, welcome to TradeBot Admin Console""")
    user_id = f"admin"
...
""")
            if choice == "list all":
                response = send_cmd({"cmd":"list all","owner":user_id, "reply_to":RPL_CH})
                traders = response.get("traders")
                print(f"""
{",".join(traders)}
""")
            if choice == "start":
...
""")
            ...
            if choice == "status":
                cancelled = False
                while True:
                    print(f"What symbol would you like the status for?: {symbol_list}")
                    symbol_choice = input("Please type the full symbol: ").upper()
                    if symbol_choice in symbol_list:
                        break
                    if symbol_choice != "EXIT":
            ...
            if choice == "heartbeat":
                print(get_heartbeat())
            if choice == "stop all":
                print(send_cmd({"cmd":"stop_all", "reply_to":RPL_CH}))
            if choice == "shutdown":
                print(send_cmd({"cmd":"shutdown", "reply_to":RPL_CH}))
            if choice == "quit":
                break
        else:
